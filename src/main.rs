...
async fn main() -> Result<()> {
    ...
    let log_file = format!("logs/dicom_sender_{}.log", session_id);
    let summary_file = format!("logs/dicom_sender_summary_{}.json", session_id);
    ...
    let mut all_results: Vec<TransferStats> = Vec::new();
